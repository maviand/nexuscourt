        ...
        <div className="w-full max-w-2xl mx-auto bg-white border border-gray-200 rounded-xl p-8 shadow-lg animate-fade-in">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                    {ERAS.map(era => (
                        <button
                            ...
                            className={`p-3 rounded-lg border text-left transition-all ${year === era.year ? 'border-nba-blue bg-blue-50 ring-1 ring-nba-blue' : 'border-gray-200 bg-gray-50 hover:border-blue-300'}`}
                        >
                            <div className="font-bold text-sm text-gray-900">{era.year}</div>
                            <div className="text-xs text-gray-600 truncate">{era.eraName}</div>
                        </button>
                    ))}
                </div>
            </div>
            ...
            <div className="mb-10 text-center">
                ...
                <input 
                    ...
                    className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-[#1d428a] mb-4"
                />
                <div className="text-6xl font-black text-nba-blue font-mono">{year}</div>
                <p className="text-gray-500 font-bold uppercase tracking-widest mt-2 text-sm">Starting Season</p>
            </div>

            <div className="bg-gray-50 rounded-xl p-6 min-h-[100px] mb-8 border border-gray-200">
                {context ? (
                    <>
                        <h3 className="text-xl font-bold text-gray-900 mb-2">{context.eraName}</h3>
                        <p className="text-gray-600 text-sm leading-relaxed">{context.description}</p>
                    </>
                ) : (
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
                <button 
                    ...
                    className={`p-4 rounded-lg border flex flex-col items-center gap-2 transition-all ${entryMethod === 'Draft' ? 'bg-blue-50 border-nba-blue text-nba-blue' : 'bg-white border-gray-200 text-gray-500 hover:border-gray-400'}`}
                >
                    <Briefcase size={24} />
                    <span className="font-bold uppercase text-sm">Enter Draft</span>
                </button>
                <button 
                    onClick={() => setEntryMethod('Specific Team')}
                    className={`p-4 rounded-lg border flex flex-col items-center gap-2 transition-all ${entryMethod === 'Specific Team' ? 'bg-blue-50 border-nba-blue text-nba-blue' : 'bg-white border-gray-200 text-gray-500 hover:border-gray-400'}`}
                >
                    <Users size={24} />
                    <span className="font-bold uppercase text-sm">Join Team</span>
                </button>
            </div>
            ...
            {entryMethod === 'Specific Team' && (
                <div className="mb-8">
                    ...
                    <select 
                        ...
                        className="w-full bg-white border border-gray-300 rounded p-4 text-gray-900 focus:border-nba-blue outline-none"
                    >
                        {NBA_TEAMS.map(team => (
            ...
            <button 
                ...
                className="w-full py-4 bg-nba-blue text-white font-bold uppercase tracking-[0.2em] hover:bg-blue-900 transition-all rounded disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3 shadow-md"
            >
...
export default function App() {
      ...
      try {
          ...
          if (player?.potential === 99 && currentAge >= 26 && currentAge <= 32) {
              if (newAttributes.rating < 99) {
                  ...
                  newAttributes.athleticism = Math.max(newAttributes.athleticism, 90);
  ...
  return (
    <div className="min-h-screen bg-[#f7f9fa] text-gray-900 font-sans selection:bg-nba-blue selection:text-white pb-20">
      ...
      {/* Navbar */}
      <nav className="sticky top-0 z-50 w-full border-b border-gray-200 bg-white shadow-sm p-4 mb-8">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-nba-blue rounded flex items-center justify-center text-white font-black font-mono shadow-md">N</div>
                <span className="text-xl font-bold tracking-tighter text-nba-blue uppercase display-font">Nexus Court</span>
            </div>
            
            <div className="flex items-center gap-6">
                 {/* Player Info (If Active) */}
                 {player && currentAttributes && step !== 'create' && (
                     <div className="hidden lg:flex items-center gap-4 px-4 py-1 bg-gray-50 rounded-full border border-gray-200">
                         <span className="text-sm font-bold text-gray-700 uppercase">{player.name}</span>
                ...
                {step !== 'create' && (
                    <button 
                        ...
                        className={`flex items-center gap-2 text-xs font-bold uppercase px-3 py-1 rounded border transition-colors ${injuriesEnabled ? 'bg-red-50 text-red-600 border-red-200' : 'bg-gray-50 text-gray-400 border-gray-200'}`}
                    >
                        <Settings size={14} /> Injuries: {injuriesEnabled ? 'ON' : 'OFF'}
                    </button>
                )}
                
                <div className="text-xs font-mono text-gray-400 flex gap-4 hidden md:flex">
                    <span className={step === 'create' ? 'text-nba-blue font-bold' : ''}>01. CREATE</span>
                    <span>/</span>
                    <span className={step === 'era' ? 'text-nba-blue font-bold' : ''}>02. ERA</span>
                    <span>/</span>
                    <span className={step === 'strategy' ? 'text-nba-blue font-bold' : ''}>03. STRATEGY</span>
                    <span>/</span>
                    <span className={step === 'season_loop' ? 'text-nba-blue font-bold' : ''}>04. SIMULATION</span>
                </div>
            </div>
        </div>
      </nav>
            ...
            <>
                {isSimulating && !lastSeason && (
                    <div className="text-center animate-pulse space-y-4">
                        <div className="w-24 h-24 border-4 border-nba-blue border-t-transparent rounded-full animate-spin mx-auto"></div>
                        <h2 className="text-2xl font-bold text-nba-blue uppercase tracking-widest">Simulating Season...</h2>
                        <p className="text-gray-500 font-mono text-sm">Processing League Outcomes...</p>
                    </div>
                )}
