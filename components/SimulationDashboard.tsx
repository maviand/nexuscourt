                                ...
                                {season.teamChemistry !== undefined && (
                                    <div className="mt-4 flex items-center gap-3">
                                        ...
                                        <div className="flex-1 h-2 bg-slate-800 rounded-full overflow-hidden">
                                            <div 
                                                className={`h-full ${
                                                    season.teamChemistry >= 80 ? 'bg-emerald-500' : 
...
export const SimulationDashboard: React.FC<Props> = ({ simulation, player, onReset }) => {
  ...
  const progressionData = simulation.seasons.map(s => ({
    ...
    Athleticism: s.ratingsSnapshot?.athleticism || 70,
    ...
    <div className="fixed inset-0 bg-slate-950 overflow-y-auto z-50">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end border-b border-slate-800 pb-8 gap-6">
            <div>
                ...
                <h1 className="text-6xl font-black text-white tracking-tighter uppercase mb-2">{player.name}</h1>
                <p className="text-slate-300 max-w-2xl text-lg leading-relaxed">{simulation.summary}</p>
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            ...
            <StatCard label="MVP Awards" value={simulation.accolades.mvps} icon={Trophy} color="text-purple-400" />
            <StatCard label="All-Star Selections" value={simulation.accolades.allStars} icon={Star} color="text-blue-400" />
            <StatCard label="Hall of Fame Odds" value={`${simulation.accolades.hallOfFameChance}%`} icon={Award} color="text-green-400" />
        </div>
        ...
        <div className="flex justify-center pt-8">
            <button 
                onClick={onReset}
                className="px-10 py-4 bg-white text-black font-black uppercase tracking-[0.2em] hover:bg-yellow-400 hover:text-black transition-all rounded-sm shadow-lg transform hover:-translate-y-1"
            >
                Start New Career
            </button>
        </div>
